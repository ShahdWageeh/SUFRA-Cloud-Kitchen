...
function Rating({ value, compact = false }) {
  return (
    <span className="inline-flex items-center gap-1 text-primary">
      <FontAwesomeIcon icon={faStar} className={compact ? "h-2.5 w-2.5" : "h-3 w-3"} />
  ...
  return (
    <article className="overflow-hidden rounded-md bg-white shadow-[0_12px_35px_rgba(27,28,28,0.08)] ring-1 ring-primary/10">
      <div className="relative aspect-[1.45] overflow-hidden">
        <Image src={meal.image} alt={meal.name} fill sizes="(max-width: 768px) 100vw, 31vw" className="object-cover" />
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-3">
          <h3 className="text-sm font-bold leading-snug text-text-primary">{meal.name}</h3>
          <Rating value={meal.rating} compact />
        </div>
        <p className="mt-2 min-h-10 text-xs leading-5 text-text-secondary">{meal.description}</p>
    ...
    <article className="rounded-md bg-white p-5 shadow-sm ring-1 ring-primary/10">
      <div className="flex gap-4">
        <div className="relative h-14 w-14 shrink-0 overflow-hidden rounded-md">
          <Image src={review.image} alt={review.author} fill sizes="56px" className="object-cover" />
        </div>
        <div>
          <div className="mb-1 flex gap-0.5 text-primary">
            {Array.from({ length: review.rating }).map((_, index) => (
              <FontAwesomeIcon key={index} icon={faStar} className="h-2.5 w-2.5" />
            ))}
          </div>
          <p className="text-xs leading-5 text-text-secondary">{review.quote}</p>
          <p className="mt-3 text-xs font-bold text-text-primary">{review.author}</p>
    ...
    <main className="min-h-screen bg-background text-text-primary">
      <section className="mx-auto max-w-6xl px-4 pt-4 sm:px-6 lg:px-8">
        <div className="relative h-52 overflow-hidden rounded-t-md sm:h-64 lg:h-72">
          <Image src={chef.coverImage} alt={`${chef.brandName} kitchen cover`} fill priority sizes="100vw" className="object-cover" />
        </div>
        ...
        <div className="relative mx-auto -mt-10 max-w-5xl rounded-md bg-white p-5 shadow-[0_14px_40px_rgba(27,28,28,0.10)] ring-1 ring-primary/10 sm:p-6">
          <div className="grid gap-5 lg:grid-cols-[auto_1fr_auto] lg:items-start">
            <div className="relative h-24 w-24 overflow-hidden rounded-full border-4 border-white shadow-lg">
              <Image src={chef.avatar} alt={chef.brandName} fill sizes="96px" className="object-cover" />
            ...
            <div>
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                ...
                <div className="flex gap-2 lg:hidden">
                  <button className="rounded-full border border-primary px-4 py-2 text-xs font-semibold text-primary transition hover:bg-primary hover:text-white">
                    {chef.actions.follow}
                  </button>
                  <button className="rounded-full bg-primary px-4 py-2 text-xs font-semibold text-white transition hover:bg-primary-container">
                    {chef.actions.order}
                  </button>
                </div>
              </div>
              <p className="mt-3 max-w-3xl text-sm leading-6 text-text-secondary">{chef.bio}</p>

              <div className="mt-4 grid grid-cols-3 gap-3 sm:max-w-lg">
                {chef.stats.map((stat) => (
                  <div key={stat.id} className="flex items-center gap-2 rounded-md bg-secondary-container px-3 py-2">
                    <span className="flex h-8 w-8 items-center justify-center rounded-md bg-primary/10 text-primary">
                      <FontAwesomeIcon icon={statIconMap[stat.id]} className="h-3.5 w-3.5" />
                    </span>
                    <span>
                      <span className="block text-sm font-bold">{stat.value}</span>
                      <span className="block text-[10px] uppercase tracking-wide text-text-secondary">{stat.label}</span>
                    </span>
                  </div>
                ))}
              </div>
            ...
            <div className="hidden gap-2 lg:flex">
              <button className="rounded-full border border-primary px-5 py-2 text-xs font-semibold text-primary transition hover:bg-primary hover:text-white">
                {chef.actions.follow}
              </button>
              <button className="rounded-full bg-primary px-5 py-2 text-xs font-semibold text-white transition hover:bg-primary-container">
                {chef.actions.order}
              </button>
            </div>
          </div>
      ...
      <section className="mx-auto max-w-6xl px-4 py-12 sm:px-6 lg:px-8">
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wide text-primary">{offerings.eyebrow}</p>
        ...
        <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 lg:px-8">
          <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-text-secondary">{reviews.subtitle}</p>
            </div>
