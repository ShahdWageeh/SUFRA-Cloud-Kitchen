...
export default function DeliverySidebar({ isOpen, onClose }) {
  ...
  const chefName = user?.firstName
    ...
    : "Delivery";
  const chefTitle = user?.kitchenName || user?.role || "Delivery";
    ...
    <>
      ...
      >
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-[#7e6a63]">Delivery Portal</p>
        </div>
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 rounded-xl px-4 py-3 text-sm transition ${
                  active
                    ? "border-l-2 border-primary bg-[#f6efed] font-medium text-primary"
        ...
        <div className="border-t border-[#e9dfdc] pt-5">
          {/* <Link
            ...
            onClick={onClose}
            className="mb-4 block w-full rounded-xl bg-primary py-3 text-center text-[14px] font-bold text-white transition hover:bg-[#7d3820]"
          >
            Create New Listing
          </Link> */}
          ...
          >
            <div className="flex items-center gap-3">
              <div className="flex h-11 w-11 items-center justify-center rounded-full bg-linear-to-br from-[#d7a27f] to-[#8f4e32] text-xs font-semibold text-white">
                {initials}
              </div>
